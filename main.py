...
def spe(con_mat, n):
    ...
    spe = []
    for i in range(n):
        tn = np.sum(con_mat) - np.sum(con_mat[i, :]) - np.sum(con_mat[:, i]) + con_mat[i, i]
...
def train_and_eval(datadir, datname, hyperpm):
    ...
    input_data = data[:,:-1]
    label = data[:,-1]-1
    skf = StratifiedKFold(n_splits=10, random_state=10, shuffle=True)#shang8xia4zuihou1
    set_rng_seed(hyperpm.seed)
    ...
    for train_index, test_index in skf.split(input_data, label):
        clk += 1
        agent = EvalHelper(input_data_dims, input_data, label, hyperpm, train_index, test_index) #input_data.shape=(598,365) label.ahape=(598,)
        tm = time.time()
        ...
        for t in range(hyperpm.nepoch):
            ...
            if cur_val_acc > best_val_acc:
                ...
                model_sav.close()
                model_sav = tempfile.TemporaryFile()
                dict_list = [agent.MessagePassing0.state_dict(),
        ...
        agent.MessagePassing0.load_state_dict(dict_list[0])
        agent.ModalFusion.load_state_dict(dict_list[1])
        ...
        spes.append(spe(conf_mat, hyperpm.nclass))
        print('sens:',sens)
        print('spes:',spes)
    ...
    print('SPE_mean:', np.array(spes).mean(axis=0), 'SPE_std:', np.array(spes).std(axis=0))
...
def main(args_str=None):
    ...
    # parser.add_argument('--gpu', type=int, default=-3, metavar='N', help='the GPU number (default auto schedule)')
    parser.add_argument('--gpu', default='1', help='gpu id')
    parser.add_argument('--nepoch', type=int, default=450,
                        help='Max number of epochs to train.')
    parser.add_argument('--early', type=int, default=200,
