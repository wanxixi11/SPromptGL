...
def smooth_CE(output, labels, idx, peak):
    batch, num_cls = output[idx].shape
    label_logits = np.zeros(output[idx].shape, dtype=np.float32) + (1-peak)
    # /(num_cls-1)
    # breakpoint()
    for i in range(batch):
        if labels[idx][i] == 1:
            label_logits[i][1] = peak
        else:
            label_logits[i][0] = peak
   # breakpoint()
    smooth_label = torch.from_numpy(label_logits).to(output[idx].device)

    output_clone = output[idx].clone()
    output_clone = F.log_softmax(output_clone, -1)
    ce = torch.mul(output_clone, smooth_label)
...
#     # 数值稳定性处理
#     output_clone = output[idx].clone()
#     output_clone = torch.log(torch.clamp(output_clone, min=1e-10))
#     output_clone = F.log_softmax(output_clone, -1)
    
#     smooth_label = torch.from_numpy(label_logits).to(output_clone.device)
